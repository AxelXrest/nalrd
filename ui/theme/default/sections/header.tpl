...
<body id="app" class="app off-canvas nav-expand">
	...
	<header class="site-head" id="site-head" style="background:#005ACC; ">
	...
	<div class="main-container clearfix" style=" color:black">
		<aside style=" color:black" class="nav-wrap nav-expand" id="site-nav" data-perfect-scrollbar>
			<div class="nav-head" style="background:#005ACC;">
			...
			<nav class="site-nav clearfix" role="navigation" style="background:white; color:black">
				{if $_admin['user_type'] eq 'Admin' || $_admin['user_type'] eq 'Sales' || $_admin['user_type'] eq 'Regular' ||  $_admin['user_type'] eq 'POS'}
					<div class="profile clearfix mb15">
						...
						<div class="group">
							<div class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"
									aria-expanded="false">{$_admin['fullname']}<span class="caret"></a></span>
								<ul class="dropdown-menu">
									<li><a href="{$_url}settings/users-edit/{$_admin['id']}"><i class="ion ion-person"></i>
											{$_L['My_Account']}<div class="ripple-wrapper"></div></a></li>
									<li><a href="{$_url}settings/change-password"><i class="ion ion-settings"></i>
											{$_L['Change_Password']}</a></li>
									<li><a href="{$_url}logout"><i class="ion ion-power"></i> {$_L['Logout']}</a></li>
								</ul>
							</div>
							<small class="desig" style="color:black">{if $_admin['user_type'] eq 'Admin'}
								{$_L['Administrator']} {elseif $_admin['user_type'] eq 'Sales'} {$_L['Sales']}
								{elseif $_admin['user_type'] eq 'Regular'}{$_L['Regular']}
								{elseif $_admin['user_type'] eq 'POS'} {$_L['POS']}
								{/if}</small>
						</div>
					</div>
				{else}
					<div class="profile clearfix mb15">
						...
						<div class="group">
							<div class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown"
									aria-expanded="false">{$_user['fullname']}<span class="caret"></span></a>
								<ul class="dropdown-menu">
									<li><a href="{$_url}accounts/profile"><i class="ion ion-person"></i> {$_L['My_Account']}
											<div class="ripple-wrapper"></div>
										</a></li>
									<li><a href="{$_url}accounts/change-password"><i class="ion ion-settings"></i>
											{$_L['Change_Password']}</a></li>
									<li><a href="{$_url}logout"><i class="ion ion-power"></i> {$_L['Logout']}</a></li>
								</ul>
							</div>
				...
				<ul id="leftMenu" class="list-unstyled clearfix nav-list mb15" style=" color:black;">
					...
					<li class="hidden {if $_system_menu eq 'message'}open{/if}">
						...
						<ul class="inner-drop list-unstyled">
							<li {if $_system_menu eq 'message'}class="active" {/if}><a
									href="{$_url}message/inbox">{$_L['Inbox']}</a></li>
							<li {if $_system_menu eq 'message'}class="active" {/if}><a
									href="{$_url}message/outbox">{$_L['Outbox']}</a></li>
							<li {if $_system_menu eq 'message'}class="active" {/if}><a
									href="{$_url}message/compose">{$_L['Compose']}</a></li>
						</ul>
					</li>

					{if $_admin['user_type'] eq 'Regular'}
						<li {if $_system_menu eq 'customers'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								<li {if $_system_menu eq 'customers'}class="active" {/if}><a
										href="{$_url}customers/list">{$_L['List_Contact']}</a></li>
							</ul>
						</li>
						...
						<li {if $_system_menu eq 'prepaid'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								...
								<li {if $_system_menu eq 'prepaid'}class="active" {/if}><a
					...
					{if $_admin['user_type'] eq 'Admin' || $_admin['user_type'] eq 'Sales' ||  $_admin['user_type'] eq 'POS'}
						<li {if $_system_menu eq 'customers'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								...
								<li {if $_system_menu eq 'prepaid'}class="active" {/if}>
									<a href="{$_url}customers/customers_details" onClick="toggleDropdownMobile(this)">User Details</a>
								</li>
								<li {if $_system_menu eq 'prepaid'}class="active" {/if}>
									<a href="{$_url}customers/search_users" onClick="toggleDropdownMobile(this)">Search Details</a>
								</li>
							{*	<li {if $_system_menu eq 'prepaid'}class="active" {/if}>
									<a href="{$_url}customers/customers_data" onClick="toggleDropdownMobile(this)">Users Data </a>
								</li>
								
								<li {if $_system_menu eq 'prepaid'}class="active" {/if}>
									<a href="{$_url}customers/list" onClick="toggleDropdownMobile(this)">User Management</a>
								</li> *}
						...
						{if $_admin['user_type'] eq 'Admin'}
							<li {if $_system_menu eq 'services'}class="open" {/if}>
								...
								<ul class="inner-drop list-unstyled">
									<li {if $_system_menu eq 'services'}class="active" {/if}><a
						...
						<li {if $_system_menu eq 'hotspot'}class="open" {/if}>
							<a href="#" onClick="toggleDropdownMobile(this)">
								<i class="ion ion-cube"></i>
								<span class="text">{$_L['hotspot']}</span>
								<i class="arrow ion-chevron-left"></i>
							</a>
							<ul class="inner-drop list-unstyled">
							{if $_admin['user_type'] eq 'Admin' || $_admin['user_type'] eq 'Sales' || $_admin['user_type'] eq 'POS'}
								<li {if $_system_menu eq 'services'}class="active" {/if}><a
										href="{$_url}hotspot/packages">Voucher Sales</a></li>
								{/if}
								{if $_admin['user_type'] eq 'Admin'}

									<li {if $_system_menu eq 'services'}class="active" {/if}><a
											href="{$_url}hotspot/find_pack2">Batch User</a></li>
								{/if}
							</ul>
						</li>
							...
							<li {if $_system_menu eq 'network'}class="open" {/if}>
								...
								<ul class="inner-drop list-unstyled">
								{if $_admin['user_type'] eq 'Admin'}
									<li {if $_system_menu eq 'network'}class="active" {/if}><a href="{$_url}routers/list">Add
											Nas</a></li>
									<li {if $_system_menu eq 'network'}class="active" {/if}><a
											href="{$_url}pool/list">{$_L['Pool']}</a></li>
								{/if}
									<li {if $_system_menu eq 'network'}class="active" {/if}><a
											href="{$_url}ip_bind/web">IP Bind</a></li>
								</ul>
							</li>
						...
						{* <li {if $_system_menu eq 'reports'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								...
								<li {if $_system_menu eq 'reports'}class="active" {/if}><a
										href="{$_url}latestExpire/latest_expired">{$_L['latest_expired_report']}</a></li>
							</ul>
						</li> *}

						{if $_admin['user_type'] eq 'Admin' || $_admin['user_type'] eq 'Sales'}
							<li {if $_system_menu eq 'monitor_nas'}class="open" {/if}>
								<a href="{$_url}monitornas/nas">
									<i class="ion ion-clipboard"></i>
									<span class="text">{$_L['monitor_nas']}</span>
									<i class="arrow ion-chevron-left"></i>
								</a>
							</li>
						{/if}
					...
					*}
					...
					{if $_admin['user_type'] eq 'Regular'}
						<li {if $_system_menu eq 'regularwallet'}class="active" {/if}>
							<a href="{$_url}regularwallet">
								<i class="ion ion-card"></i>
								<span class="text">{$_L['wallet']}</span>
							</a>
						</li>
					{/if}


					{if $_admin['user_type'] eq 'Sales' ||  $_admin['user_type'] eq 'POS'}
						<li {if $_system_menu eq 'wallet'}class="active" {/if}>
							<a href="{$_url}sellerwallet">
								<i class="ion ion-card"></i>
								<span class="text">{$_L['wallet']}</span>
							</a>
						</li>
					...
					{if $_admin['user_type'] eq 'Admin'}
						<li {if $_system_menu eq 'adminwallet'}class="active" {/if}>
							<a href="{$_url}adminwallet">
								<i class="ion ion-card"></i>
								<span class="text">{$_L['wallet']}</span>
							</a>
						</li>
						...
						{if $_admin['user_type'] eq 'Sales' ||  $_admin['user_type'] eq 'POS'}
							<li {if $_system_menu eq 'sellerwallet'}class="active" {/if}>
								<a href="{$_url}sellerwallet">
									<i class="ion ion-card"></i>
									<span class="text">{$_L['wallet']}</span>
								</a>
						...
						{* <li {if $_system_menu eq 'pages'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								<li {if $_system_menu eq 'pages'}class="active" {/if}><a
						...
						<li {if $_system_menu eq 'settings'}class="open" {/if}>
							...
							<ul class="inner-drop list-unstyled">
								<li {if $_system_menu eq 'settings'}class="active" {/if}><a
					...
					{/if}
					{* {if $_admin['user_type'] eq 'Sales' || $_admin['user_type'] eq 'Regular' ||  $_admin['user_type'] eq 'POS'}
						<li {if $_system_menu eq 'CMS'}class="active" {/if}>
							<a href="cms/sales/dashboard.php">
								<i class="ion ion-ios-lightbulb"></i>
								<span class="text">CMS</span>
							</a>
						</li>
		...
		<div class="content-container" id="content">
			<div
				class="page {if $_system_menu eq 'dashboard'}page-dashboard{/if}{if $_system_menu eq 'reports'}page-invoice{/if}">
