...
switch ($action) {
    ...
    case 'users-post':
        ...
        if ($msg == '') {
            ...
            $e = ORM::for_table('wallet')->create();
            $e->credit_limit = $credit_limit;
            $e->credit_balance = $credit_balance;
            $e->available_balance = $available_balance;
            $e->username = $username;
            $e->last_loaded_date = $last_loaded_date;
            $e->loaded_by = $loaded_by;
    ...
    case 'dbbackup':
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            // Get the submitted batch value
            $batch = $_POST["batch"];

            // Select data from the database table
            $sql = "SELECT * FROM tbl_voucher WHERE batch='$batch'";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                header("Content-Disposition: attachment; filename=voucher_batch_$batch.xls");

                // Write the header row
                echo "<table><tr><th>ID</th><th>Type</th><th>Batch</th><th>Voucher</th><th>Generated For</th></tr>";

                // Write the data rows
                while ($row = mysqli_fetch_assoc($result)) {
                    echo "<tr><td>" . $row["id"] . "</td><td>" . $row["type"] . "</td><td>" . $row["batch"] . "</td><td>'" . $row["code"] . "'</td><td>" . $row["generated_for"] . "</td></tr>";
                }

                echo "</table>";
            } else {
                r2(U . 'settings/dbstatus', 'e', "No such batch has been created.");
            }
        }
