...
switch ($action) {
    ...
    case 'delete':
        ...
        if($delete = ORM::for_table('tbl_bandwidth')->where_id_in($array)->find_many()){

        $delete->delete();
            r2(U . 'bandwidth/list', 's', $_L['Delete_Successfully']);
        } else {
            r2(U . 'bandwidth/list', 'e', $_L['delete_problem']);
        }
        break;
    ...
    case 'add-post':
        ...
        if ($msg == '') {
            ...
            $ratetotal = $rate_up ."M" . "/" . $rate_down ."M";
            $radius = ORM::for_table('radgroupreply')->create();
			$radius->groupname = $name;
			$radius->attribute = "Mikrotik-Rate-Limit";
			$radius->value = $ratetotal;
			$radius->save();
