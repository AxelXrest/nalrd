...
switch ($action) {
	...
	case 'add-post':
		...
		$data_usage = _post('data_usage');
		$daily_quota = _post('daily_quota');
		...
		if ($msg == '') {
			...
			if ($routers == "0") {
				...
				$d->routers = $routers;
				$d->data_usage_gb = $data_usage;
				$d->daily_quota = $daily_quota;
				$d->save();

				//radius insert

				// $radius = ORM::for_table('radgroupcheck')->create();
				// $radius->groupname = $name;
				// $radius->attribute = "Framed-Protocol";
				// $radius->value = "Hotspot";
				// $radius->save();
				...
				$client->sendSync(
					$addRequest
						...
						->setArgument('on-login', '/system scheduler add name=$user interval=' . $validity . $validity_unit . ' on-event= "/ip hotspot cookie remove [find user=$user] ; /ip hotspot user remove [find name=$user] ; /ip hotspot active remove [find user=$user]; /system sche remove [find name=$user];";')
				);
	...
	case 'pppoe-delete':
		...
		try {
			...
			r2(U . 'services/pppoe', 'e', $_L['delete_problem']);
		}
	...
	case 'pppoe-add-post':
		...
		$bin = ORM::for_table('tbl_bandwidth')->where('id', $id_bw)->find_one();
		if ($d) {
			$msg .= $_L['Plan_already_exist'] . '<br>';
		}
		...
		if ($msg == '') {
			...
			// $d = ORM::for_table('radusergroup')->create();
			// $d->groupname = $bin['name_bw'];
			// $d->username= $name;
			// $d->save();
